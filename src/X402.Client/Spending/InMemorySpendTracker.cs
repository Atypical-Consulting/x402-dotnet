...
public sealed class InMemorySpendTracker : ISpendTracker
{
    ...
    private readonly Dictionary<string, decimal> spentBySymbol = new(StringComparer.OrdinalIgnoreCase);
    ...
    public void Release(AssetDescriptor asset, decimal amount)
    {
        ...
        lock (gate)
        {
            var remaining = spentBySymbol.TryGetValue(asset.Symbol, out var current)
                ? current - amount
                : -amount;
            spentBySymbol[asset.Symbol] = remaining < 0m ? 0m : remaining;
        }
    }
    ...
    {
        if (!options.TryGetLimits(asset.Symbol, out var limits))
        {
            throw new SpendingLimitExceededException(
                $"{asset.Symbol} has no spending limit declared; refusing to pay " +
                $"{Format(amount)} {asset.Symbol}.");
        }

        if (amount > limits.PerRequest)
        {
            throw new SpendingLimitExceededException(
                $"{asset.Symbol} request of {Format(amount)} exceeds the per-request limit of " +
                $"{Format(limits.PerRequest)} {asset.Symbol}.");
        }

        var alreadySpent = spentBySymbol.TryGetValue(asset.Symbol, out var current) ? current : 0m;
        var projected = alreadySpent + amount;
        if (projected > limits.PerSession)
        {
            throw new SpendingLimitExceededException(
                $"{asset.Symbol} request of {Format(amount)} would bring session spend to " +
                $"{Format(projected)}, exceeding the per-session limit of " +
    ...
    private void Add(AssetDescriptor asset, decimal amount) =>
        spentBySymbol[asset.Symbol] =
            (spentBySymbol.TryGetValue(asset.Symbol, out var current) ? current : 0m) + amount;
