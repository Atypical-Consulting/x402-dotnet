<!-- Generated by og-engine; restore this line once .github/banner.png exists. Deferred because
     og-engine's --no-pr mode pushes straight to the repository's default branch, which was not an
     action to take unreviewed and mid-branch. -->
...
- **MiCA compliance belongs to the asset's issuer, never to this library.** EURC is issued by
  Circle France SAS, licensed as an electronic money institution by the ACPR (register number
...
var myPrivateKey = Environment.GetEnvironmentVariable("X402_PRIVATE_KEY")
    ?? throw new InvalidOperationException("Set X402_PRIVATE_KEY before running.");
